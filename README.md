# MTFPlus
A plugin that adds new subclasses to NTF Cadets. [Requires EXILED](https://github.com/galaxy119/EXILED)
...
```
You don't need to have every single field written. [Below is a table with every default value](https://github.com/RogerFK/MTFPlus#default-values). If some class is missing Probability, for example, it will use a default of 100 (default values can't be edited, so make sure to change them if you don't like them). [Check the wiki clicking here to know what I mean.](https://github.com/RogerFK/MTFPlus/wiki)

### ItemManager Support, and how to use it
### :warning: EXILED doesn't currently have ItemManager. This won't work until someone does an ItemManager or something remotely close.
Below is how ItemManager will work assuming the item with name `Shotgun` is present in ItemManager
`Heavy-Shotgun.txt`
```yaml
